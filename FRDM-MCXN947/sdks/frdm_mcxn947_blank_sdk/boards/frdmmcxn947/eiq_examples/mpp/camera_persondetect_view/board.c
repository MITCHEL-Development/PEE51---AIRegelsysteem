                              ...
                              uint8_t txBuffSize)
{
    ...
    {
        status_t ret;
        uint8_t txData[2] = {subAddress, txBuff[i]};

        ret = BOARD_LPI2C_Send(base, deviceAddress, 0, 0, txData, 2);
        if (ret != kStatus_Success) {
            PRINTF("\nLPI2C error: failed Writing txBuff[%d] to slave.", i);
            return ret;
        }
    }
