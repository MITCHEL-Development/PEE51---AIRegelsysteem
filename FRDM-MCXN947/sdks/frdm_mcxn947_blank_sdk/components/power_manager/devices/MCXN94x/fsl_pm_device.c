...
static uint8_t FindOperateMode(uint32_t rescIndex, pm_resc_group_t *pSysRescGroup)
{
    ...
    uint8_t u8Tmp = (uint8_t)(uint32_t)(PM_RESC_GROUP(pSysRescGroup, rescIndex));
...
void EnableResources(pm_resc_mask_t *pSoftRescMask, pm_resc_group_t *pSysRescGroup)
{
    ...
    for (i = 0UL; i < (uint32_t)PM_CONSTRAINT_COUNT; i++)
    {
...
static void SetSRAMOperateMode(uint8_t operateMode, resource_recode_t *pResourceRecode)
{
    ...
    sramId = (uint32_t)(pResourceRecode - resourceDB);
    assert(sramId >= (uint32_t)kResc_SRAM_RAMA0_8K);
    assert(sramId <= (uint32_t)kResc_SRAM_FLEXSPI0);
    ...
    {
        if (sramId >= (uint32_t)kResc_SRAM_LPCAC)
        {
            sramId += 9U;
        }
        /* Calculate the bit shift value for SRAMDIS/SRAMRET registers */
        sramId -= (uint32_t)kResc_SRAM_RAMX0_32K;
        sramId = 1UL << sramId;
...
static void SetAnalogOperateMode(uint8_t operateMode, resource_recode_t *pResourceRecode)
{
    ...
    analogId = (uint8_t)(pResourceRecode - resourceDB);
    assert(analogId >= (uint8_t)kResc_VREF);
    assert(analogId <= (uint8_t)kResc_CMP2);

    switch ((resc_name_t)analogId)
    {
        case kResc_VREF:
            if (operateMode == PM_RESOURCE_FULL_ON)
            {
                CLOCK_EnableClock(kCLOCK_Vref);
                SPC_EnableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlVref);
                SPC_EnableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlVref);
                /* Note: The user could add xxx IP Init function here. */
            }
            else
            {
                CLOCK_DisableClock(kCLOCK_Vref);
                SPC_DisableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlVref);
                SPC_DisableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlVref);
        ...
        case kResc_USB3V_DET:
            if (operateMode == PM_RESOURCE_FULL_ON)
            {
                SPC_EnableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlUsb3vDet);
                SPC_EnableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlUsb3vDet);
            }
            else
            {
                SPC_DisableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlUsb3vDet);
                SPC_DisableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlUsb3vDet);
            }
            break;
        ...
        case kResc_DAC0:
            if (operateMode == PM_RESOURCE_FULL_ON)
            {
                ...
                SPC_EnableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlDac0);
                SPC_EnableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlDac0);
            }
            else
            {
                CLOCK_DisableClock(kCLOCK_Dac0);
                SPC_DisableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlDac0);
                SPC_DisableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlDac0);
            }
            break;
        ...
        case kResc_DAC1:
            if (operateMode == PM_RESOURCE_FULL_ON)
            {
                ...
                SPC_EnableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlDac1);
                SPC_EnableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlDac1);
            }
            else
            {
                CLOCK_DisableClock(kCLOCK_Dac1);
                SPC_DisableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlDac1);
                SPC_DisableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlDac1);
            }
            break;
        ...
        case kResc_DAC2:
            if (operateMode == PM_RESOURCE_FULL_ON)
            {
                ...
                SPC_EnableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlDac2);
                SPC_EnableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlDac2);
            }
            else
            {
                CLOCK_DisableClock(kCLOCK_Dac2);
                SPC_DisableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlDac2);
                SPC_DisableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlDac2);
            }
            break;
        ...
        case kResc_OPAMP0:
            if (operateMode == PM_RESOURCE_FULL_ON)
            {
                ...
                SPC_EnableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlOpamp0);
                SPC_EnableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlOpamp0);
            }
            else
            {
                CLOCK_DisableClock(kCLOCK_Opamp0);
                SPC_DisableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlOpamp0);
                SPC_DisableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlOpamp0);
            }
            break;
        ...
        case kResc_OPAMP1:
            if (operateMode == PM_RESOURCE_FULL_ON)
            {
                ...
                SPC_EnableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlOpamp1);
                SPC_EnableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlOpamp1);
            }
            else
            {
                CLOCK_DisableClock(kCLOCK_Opamp1);
                SPC_DisableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlOpamp1);
                SPC_DisableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlOpamp1);
            }
            break;
        ...
        case kResc_OPAMP2:
            if (operateMode == PM_RESOURCE_FULL_ON)
            {
                ...
                SPC_EnableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlOpamp2);
                SPC_EnableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlOpamp2);
            }
            else
            {
                CLOCK_DisableClock(kCLOCK_Opamp2);
                SPC_DisableActiveModeAnalogModules(SPC0, (uint32_t)kSPC_controlOpamp2);
                SPC_DisableLowPowerModeAnalogModules(SPC0, (uint32_t)kSPC_controlOpamp2);
            }
            break;
        ...
        case kResc_CMP0:
            if (operateMode == PM_RESOURCE_FULL_ON)
            {
                SPC_EnableActiveModeAnalogModules(SPC0, ((uint32_t)kSPC_controlCmp0 | (uint32_t)kSPC_controlCmp0Dac));
                SPC_EnableLowPowerModeAnalogModules(SPC0, ((uint32_t)kSPC_controlCmp0 | (uint32_t)kSPC_controlCmp0Dac));
            }
            else
            {
                SPC_DisableActiveModeAnalogModules(SPC0, ((uint32_t)kSPC_controlCmp0 | (uint32_t)kSPC_controlCmp0Dac));
                SPC_DisableLowPowerModeAnalogModules(SPC0,
                                                     ((uint32_t)kSPC_controlCmp0 | (uint32_t)kSPC_controlCmp0Dac));
            }
            break;
        ...
        case kResc_CMP1:
            if (operateMode == PM_RESOURCE_FULL_ON)
            {
                SPC_EnableActiveModeAnalogModules(SPC0, ((uint32_t)kSPC_controlCmp1 | (uint32_t)kSPC_controlCmp1Dac));
                SPC_EnableLowPowerModeAnalogModules(SPC0, ((uint32_t)kSPC_controlCmp1 | (uint32_t)kSPC_controlCmp1Dac));
            }
            else
            {
                SPC_DisableActiveModeAnalogModules(SPC0, ((uint32_t)kSPC_controlCmp1 | (uint32_t)kSPC_controlCmp1Dac));
                SPC_DisableLowPowerModeAnalogModules(SPC0,
                                                     ((uint32_t)kSPC_controlCmp1 | (uint32_t)kSPC_controlCmp1Dac));
            }
            break;
        ...
        case kResc_CMP2:
            if (operateMode == PM_RESOURCE_FULL_ON)
            {
                ...
                SPC_EnableActiveModeAnalogModules(SPC0, ((uint32_t)kSPC_controlCmp2 | (uint32_t)kSPC_controlCmp2Dac));
                SPC_EnableLowPowerModeAnalogModules(SPC0, ((uint32_t)kSPC_controlCmp2 | (uint32_t)kSPC_controlCmp2Dac));
            }
            else
            {
                CLOCK_DisableClock(kCLOCK_Cmp2);
                SPC_DisableActiveModeAnalogModules(SPC0, ((uint32_t)kSPC_controlCmp2 | (uint32_t)kSPC_controlCmp2Dac));
                SPC_DisableLowPowerModeAnalogModules(SPC0,
                                                     ((uint32_t)kSPC_controlCmp2 | (uint32_t)kSPC_controlCmp2Dac));
            }
            break;
...
static status_t ManageWakeupSource(pm_wakeup_source_t *ws, bool enable)
{
    ...
    if (inputType == (uint32_t)kWup_inputType_extPin)
    {
        ...
        if (enable)
        {
            ...
            (void)EnableIRQ(WUU_IRQn);
        }
        else
        {
            (void)DisableIRQ((IRQn_Type)irqn);
            (void)DisableIRQ(WUU_IRQn);
        }
    }
    else
    {
